...
impl VoteProof {
    ...
    ///         = a
    /// ```
    /// 6. The observer can verify that `b = g2*r + R*c'`. This holds, as:
    /// ```equation
    ///           g2*r + R*c'
    ...
    /// 1. We pre-select our sub-challenge `c'` and response `r` by randomly generating them.
    /// 2. We calculate `a = g1*r + Y*c'`, where `Y` is either `Z` or `Z - g1` depending on whether
    ///    we are trying to fake v=0 or v=1 respectively.
    /// 3. We calculate `b = g2*r + R*c'`.
    /// 4. The observer can verify that `a = g1*r + Y*c'` and `b = g2*r + R*c'`; these trivially
...
mod tests {
    ...
    fn test_vote() {
        ...
        assert!(vote1.pwf.verify(&election, &vote1.Z, &vote1.R, "1", "Alice").is_some());

        let vote2 = election.create_vote(&mut rng, "1", "Bob", false);
        assert!(vote2.pwf.verify(&election, &vote2.Z, &vote2.R, "1", "Bob").is_some());
